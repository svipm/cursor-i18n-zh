...
- `scripts\install.cmd`: 一键安装或更新汉化.
- `scripts\restore.cmd`: 一键恢复原版.
- `scripts\status.cmd`: 查看当前状态.
...
`npm run check` 或 GUI 的 `安全检查` 不会修改 Cursor. 它会执行这些检查:
...
- 每次构建都会上传 artifact.
- 推送 `v*` 标签时会自动把 zip 附加到 GitHub Release.
