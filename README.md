...
面向 Windows 的开源软件汉化工作台. 当前支持 Cursor 和 Claude Desktop, 提供应用检测, 当前版本强制备份, SHA256 校验, 汉化安装, 原版恢复, Cursor 用量监控和非强制更新提示.

本项目是第三方开源工具, 不是 Cursor 或 Anthropic 官方项目. 使用前请保存正在编辑的内容, 并确认你有权修改本机软件资源文件.

## 软件截图

### 软件中心

![汉化工作台软件中心](assets/screenshots/workbench-software-center.png)

工作台按实际资源结构识别 Cursor 和 Claude Desktop, 不使用固定软件版本白名单. 截图中的账号和本机路径信息已经遮挡.

### 备份历史和一键恢复

![汉化工作台备份历史](assets/screenshots/workbench-backups.png)

每条记录都会显示软件版本, 创建时间, 文件数量和完整性状态. 只有与当前安装版本匹配且校验通过的备份可以一键恢复.

### Cursor 用量监控

![汉化工作台 Cursor 用量监控](assets/screenshots/workbench-usage.png)

用量信息由 Rust 后端只读本机 Cursor 登录状态并请求 Cursor 官方接口. 登录令牌不会传给前端, 不写日志, 不写入工作台文件.

### Cursor 中文设置和套餐用量

![Cursor 中文设置和套餐用量](assets/screenshots/cursor-localized-settings.png)

截图基于 Cursor 3.11.19. Cursor 专有新功能会随上游版本变化, 新出现的文案可能需要继续补充词典.
...
GitHub Actions 会执行 Node.js 测试, 词典校验, Rust 测试, Release 构建, ZIP 冒烟测试和 SHA256 生成. 推送 `v*` 标签时自动创建 GitHub Release.
...
- [Stack-Cairn/LiveAgent](https://github.com/Stack-Cairn/LiveAgent): 桌面 GUI 信息架构调研参考, MIT.
