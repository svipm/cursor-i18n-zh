...
- 修复 Cursor 3.16 起工作台迁移到 React 后, 账号设置页无法再嵌入原生套餐与用量入口的问题. 该问题使账号用量门禁自 Cursor 3.16.29 起持续失败, 累积多个未关闭的兼容性 Issue.
- 修复 Cursor 3.22.7 将用量页日志前缀 `[SettingsPlanUsageTab]` 改为 `[PlanUsageConfig]` 后用量组件无法定位的问题.
...
- 账号用量嵌入同时支持旧 SolidJS 结构与 React 结构: 旧结构仍按页面级兄弟节点插入; React 结构改为向账号设置列表追加一个渲染原生用量组件的条目. 元素工厂函数名与用量组件名均从产物推导, 不硬编码压缩后的符号.
- 账号页锚点改为按结构定位, 通过账号列表锚点区分渲染代码与设置项注册表中的同名描述, 避免把用量入口插到设置项元数据里.
- 括号配对与函数体边界改用 `acorn` 分词计算, 正确跳过字符串、模板字面量、正则与注释, 结构异常时安全停止并报告具体原因, 不做猜测性写入.
- 根据 Cursor 3.22.7 实际扫描结果补齐首次转向提示、通行密钥不支持提示、报告问题菜单、glass 布局的资源管理器与终端标签、Cursor Blame 悬停延迟设置以及 Cursor 认证 MDM 策略说明文案.
- 将稳定版兼容基线从 Cursor 3.14.27 推进到 3.22.7.

### 验证

- Node.js 自动化测试 105 项全部通过, 其中新增 4 项覆盖 React 结构的用量嵌入、元素工厂推导、用量组件缺失时安全停止以及列表缺失时的结构原因上报, 并保留旧 SolidJS 结构测试.
- 使用官方签名安装包 (Anysphere, Inc.) 在隔离目录安装 Cursor 3.22.7, `product.json` commit 与官方一致, 执行完整备份与汉化安装均成功.
- 简体中文与繁體中文兼容门禁均通过: 代码替换 12976 处, 工作台入口 3 个, 账号用量入口 2 个, Cursor NLS 180 条.
- 装机验证补丁后 bundle 在实际运行中加载执行, 日志无 JavaScript 语法或解析错误, 界面显示中文.
...
[0.4.3]: https://github.com/svipm/cursor-i18n-zh/compare/v0.4.2...v0.4.3
